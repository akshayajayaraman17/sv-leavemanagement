...
13. `supabase/migration-reseed-leave-types.sql` — **run this if `leave_types` is ever empty** (check: `select * from leave_types` in the SQL Editor). `schema.sql` only seeds it once at initial setup; if a database gets re-initialized later without that step, every leave-balance calculation and every leave-type dropdown in the app silently breaks, since `get_leave_balance()` joins against this table. Also deactivates "Casual Leave" — the company's actual policy only has Annual and Medical leave. Safe to re-run any time — it upserts by `code`, so it can't create duplicates
14. `supabase/migration-approver-admin-fallback.sql` — **fixes a bug where approvals silently went to no one.** `get_approver()` returned `NULL` for any employee with neither an `approver_config` entry nor an active `manager_id` — their leave/comp-off/timesheet/regularization requests then got `approver_id = NULL` and were invisible to everyone, including admins, since every "pending for me" query filters on `approver_id = auth.uid()`. Now falls back to the longest-serving active admin. Also backfills `approver_id` on any pending request that's already stuck this way, so it shows up immediately

All fourteen are idempotent (`if not exists` / `create or replace` / `drop policy if exists`), so re-running any of them is safe. You should see "Success. No rows returned" after each.
...
- `send-notification` — emails the assigned approver when a leave/comp-off/timesheet/regularization request is submitted, and emails the employee when it's approved or rejected. **Optional** — the app works fully without it; skip it if you don't want email notifications yet. (If a requester has no approver assigned, the submit email is silently skipped — fix the approver setup, it's not a function error.)
...
- **send-notification** builds every email server-side from a record the caller's own JWT can already see (RLS still applies) — the client can only say *which* existing record to notify about and *which* event (`submitted`/`decision`), never supply arbitrary recipient addresses or message content; the recipient is always derived from the record (assigned approver for `submitted`, requester for `decision`)
